...
const ProductsList = React.memo(({ products, onAddToCart, onRemoveFromCart, onProductClick, checkProductInCart, getCartItemQuantity }) => (
...
export function ProductsSanguches() {
  ...
  // Estado inicial de filtros sincronizado con la URL y el store
  const [filters, setFilters] = useState(() => {
    const categoryFromUrl = queryParams.get('category');
    const categoriesFromUrl = queryParams.get('categories')?.split(',');
    return {
      minPrice: 0,
      category: categoryFromUrl || selectedCategory || 'all',
      categories: categoriesFromUrl || [selectedCategory || 'all']
    };
  });

  // Efecto para sincronizar URL con el estado de filtros
  useEffect(() => {
    ...
    if (category || categories) {
      ...
      setSelectedCategory(category || categories?.[0] || 'all');
    }
  }, [location.search, setSelectedCategory]);

  // Fetch products con manejo de categorías mejorado
  const fetchProducts = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const categoriesToFetch = filters.categories.includes('all') ? ['all'] : filters.categories;
      const data = await getProductsByCategories(categoriesToFetch);
      if (!data) {
  ...
  // Manejador de cambio de filtros mejorado
